...
class InvertibleLinearResNetBlock(nn.Module):
    def __init__(self, in_size, hid_size):
        super().__init__()
        self.res = nn.Sequential(
            nn.ELU(),
            spectral_norm(nn.Linear(in_size, hid_size)),
            nn.ELU(),
            spectral_norm(nn.Linear(hid_size, in_size)),
        )
...
class InvertibleLinearResNet(nn.Module):
    def __init__(self, in_size, hid_size, layers=1):
        super().__init__()
        self.res = nn.Sequential(
...
class ResLinearBlock(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        self.linear = nn.Sequential(
            ...
            nn.Dropout(),
            nn.LeakyReLU(.2),
            spectral_norm(nn.Linear(out_size, out_size)),
            nn.Dropout(),
            nn.LeakyReLU(.2),
        )
...
class GBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        if in_channels != out_channels:
            self.convx = spectral_norm(nn.Conv1d(
...
class GridDecoder(nn.Module):
    def __init__(self, dim_z, channels, start_len=16, squash=None):
        ...
        self.linear = spectral_norm(nn.Linear(dim_z, channels[0] * start_len))
        ...
        self.output = nn.Sequential(
            ...
            spectral_norm(nn.Conv1d(
                channels[-2], channels[-1], kernel_size=3, padding=1)),
        )
        self.squash = squash
...
class GridEncoder(nn.Module):
    def __init__(self, channels, out_dim=1):
        ...
        self.linear = spectral_norm(nn.Linear(channels[-1], out_dim))
